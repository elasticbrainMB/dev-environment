...
| **3 — First real project** | **Started 2026-09-12: job search triage.** OpenClaw takes over fit scoring on rows the existing job-search pipeline already collects; n8n/the existing task keeps collecting, the Google Apps Script webhook stays the only write path to the Job Tracker, and the 06:00 resume autopilot is untouched. Displaced newsletter health reporting because Matt has stated he wants the job search moved off Claude, where the newsletter idea was a prior thread's inference. Explicitly excludes the LinkedIn browser steps. **All four opening gates checked as of 2026-09-13** (Phase 2 close-out; `script.google.com` confirmed reachable from the OpenClaw container; scoring rubric extracted verbatim to `scoring-rubric.md` from the live Cowork prompt Matt pasted 2026-09-12; webhook queue check run 2026-09-13). **Gate 2's answer opens a new item:** the webhook exposes exactly two curated queues, `fit10` (81 rows) and `interested` (4 rows) — no all-rows or unscored queue exists. That blocks the shadow rollout as originally written (comparing across the real score range), not just the gate — needs a small Apps Script change (Matt's to make; no tool access to that editor from here) or a narrower shadow scope. Not decided yet. **Resolved, not rotated:** the live webhook URL landed in this session's context via a pasted task prompt 2026-09-12; Matt reviewed and chose not to rotate the Apps Script deployment — low risk, his call. Two guardrails stand before any building: shadow runs capped at ~10 rows, no Job Tracker writes until Matt confirms a backup. Scoped in `PHASE3-job-search-triage.md` |
